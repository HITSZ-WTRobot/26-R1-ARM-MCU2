...
controllers::MotorVelController::Config make_clamp_out_vel_ctrl_cfg()
{
    controllers::MotorVelController::Config cfg{};
    cfg.pid.Kp             = 30.0f;
    cfg.pid.Ki             = 0.1f;
    cfg.pid.Kd             = 15.0f;
    cfg.pid.abs_output_max = 3000.0f;
    return cfg;
}

controllers::MotorVelController::Config make_clamp_yaw_vel_ctrl_cfg()
{
    controllers::MotorVelController::Config cfg{};
    cfg.pid.Kp             = 100.0f;
    cfg.pid.Ki             = 0.8f;
...
controllers::MotorVelController::Config make_clamp_roll_vel_ctrl_cfg()
{
    controllers::MotorVelController::Config cfg{};
    cfg.pid.Kp             = 25.0f;
    cfg.pid.Ki             = 0.15f;
...
controllers::MotorVelController::Config make_clamp_catch_vel_ctrl_cfg()
{
    controllers::MotorVelController::Config cfg{};
    cfg.pid.Kp             = 20.0f;
    cfg.pid.Ki             = 1.5f;
...
controllers::MotorPosController::Config make_clamp_out_pos_ctrl_cfg()
{
    controllers::MotorPosController::Config cfg{};
    cfg.velocity_pid.Kp             = 30.0f;
    cfg.velocity_pid.Ki             = 0.10f;
    cfg.velocity_pid.Kd             = 15.00f;
    cfg.velocity_pid.abs_output_max = 4000.0f;

    cfg.position_pid.Kp             = 2.0f;
    cfg.position_pid.Ki             = 0.0085f;
    cfg.position_pid.Kd             = 2.5f;
    cfg.position_pid.abs_output_max = 100.0f;

    cfg.pos_vel_freq_ratio = 10;
    return cfg;
}

controllers::MotorPosController::Config make_clamp_roll_pos_ctrl_cfg()
{
    controllers::MotorPosController::Config cfg{};
    cfg.velocity_pid.Kp             = 30.0f;
    cfg.velocity_pid.Ki             = 0.10f;
    cfg.velocity_pid.Kd             = 15.0f;
    cfg.velocity_pid.abs_output_max = 5000.0f;

    cfg.position_pid.Kp             = 2.0f;
    cfg.position_pid.Ki             = 0.0085f;
    cfg.position_pid.Kd             = 2.50f;
    cfg.position_pid.abs_output_max = 30.0f;

    cfg.pos_vel_freq_ratio = 10;
    return cfg;
}

controllers::MotorPosController::Config make_clamp_yaw_pos_ctrl_cfg()
{
    controllers::MotorPosController::Config cfg{};
    cfg.velocity_pid.Kp             = 12.0f;
    cfg.velocity_pid.Ki             = 0.20f;
    cfg.velocity_pid.Kd             = 5.00f;
    cfg.velocity_pid.abs_output_max = 8000.0f;

    cfg.position_pid.Kp             = 80.0f;
    cfg.position_pid.Ki             = 1.00f;
    cfg.position_pid.Kd             = 0.00f;
...
controllers::MotorPosController::Config make_clamp_catch_pos_ctrl_cfg()
{
    controllers::MotorPosController::Config cfg{};
    cfg.velocity_pid.Kp             = 30.0f;
    cfg.velocity_pid.Ki             = 0.10f;
    cfg.velocity_pid.Kd             = 15.0f;
    cfg.velocity_pid.abs_output_max = 2000.0f;
...
{
    clamp_out_vel = new Motor_VelCtrl_t(motor_clamp_out, make_clamp_out_vel_ctrl_cfg());
    clamp_yaw_vel = new Motor_VelCtrl_t(motor_clamp_yaw, make_clamp_yaw_vel_ctrl_cfg());
    clamp_rollleft_vel = new Motor_VelCtrl_t(motor_clamp_roll_left, make_clamp_roll_vel_ctrl_cfg());
    clamp_rollright_vel = new Motor_VelCtrl_t(motor_clamp_roll_right, make_clamp_roll_vel_ctrl_cfg());
    clamp_catch_vel = new Motor_VelCtrl_t(motor_clamp_catch, make_clamp_catch_vel_ctrl_cfg());
    clamp_out_pos = new Motor_PosCtrl_t(motor_clamp_out, make_clamp_out_pos_ctrl_cfg());
    clamp_rollleft_pos = new Motor_PosCtrl_t(motor_clamp_roll_left, make_clamp_roll_pos_ctrl_cfg());
    clamp_rollright_pos = new Motor_PosCtrl_t(motor_clamp_roll_right, make_clamp_roll_pos_ctrl_cfg());
    clamp_yaw_pos = new Motor_PosCtrl_t(motor_clamp_yaw, make_clamp_yaw_pos_ctrl_cfg());
    clamp_catch_pos = new Motor_PosCtrl_t(motor_clamp_catch, make_clamp_catch_pos_ctrl_cfg());
...
void Clamp_softTIM(void* arguement)
{
    ...
    if ((osEventFlagsWait(flags_id, 0x00000004U, osFlagsWaitAny, 0) & 0xFF000004U) == 0x00000004U)
    {
        static int toggle = 0;
        if (toggle == 0)
        {

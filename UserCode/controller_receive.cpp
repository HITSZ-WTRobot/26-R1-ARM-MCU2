...
void controller_task(void* argument)
{
    for (;;)
    {
        if ((osEventFlagsWait(flags_id, 0x00000010U, osFlagsWaitAny, 0) & 0xFF000010U) ==
            0x00000010U)
        {
            joystick_mode = (JOYSTICK_MODE_E)(((int)joystick_mode + 1) % 3); // 三种模式循环切换
        }
...
void TIM10_Callback(TIM_HandleTypeDef* htim)
{
    (void)htim;
    soft_dog++;         // 1ms喂一次狗
    if (soft_dog > 100) // 超过0.1秒没有收到遥控器数据，认为断联
    {
        clamp_vel_out           = 0;
        clamp_vel_roll          = 0;
        clamp_vel_yaw           = 0;
